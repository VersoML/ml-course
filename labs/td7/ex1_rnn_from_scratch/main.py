"""
Exercice 1 : Implémenter un RNN from scratch avec NumPy.
...
from simple_rnn import SimpleRNN, numerical_gradient
from flax_rnn import jax_rnn_forward


# ============================================================
# PARTIE 1 : TEST DE LA CELLULE RNN (un pas de temps)
# ============================================================
np.random.seed(42)
...
np.random.seed(123)
rnn_check = SimpleRNN(input_size=2, hidden_size=3, output_size=1)

T_check = 4
X_check = np.random.randn(T_check, 2, 1)
targets_check = np.random.randn(T_check, 1, 1)

outputs_check, hiddens_check = rnn_check.forward(X_check)
grads = rnn_check.backward(X_check, outputs_check, hiddens_check, targets_check,
                            clip_value=1e10)
...
param_names = ["W_xh", "W_hh", "W_hy", "b_h", "b_y"]
grad_keys = ["dW_xh", "dW_hh", "dW_hy", "db_h", "db_y"]

for pname, gkey in zip(param_names, grad_keys):
    grad_num = numerical_gradient(rnn_check, X_check, targets_check, pname)
    grad_ana = grads[gkey]
...
for epoch in range(n_epochs):
    ...
    for i in range(0, len(signal) - seq_length - 1, seq_length):
        ...
        grads = rnn.backward(X_seq, outputs, hiddens, Y_seq)
...
plt.title("Convergence de l'entraînement (RNN from scratch)")
plt.xlabel("Époque")
plt.ylabel("Loss (MSE)")
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig("rnn_scratch_loss.png", dpi=150, bbox_inches="tight")
plt.show()
...
preds, _ = rnn.forward(X_test)
preds = preds.flatten()

plt.figure(figsize=(10, 4))
plt.plot(Y_test, label="Réel", color="steelblue", linewidth=2)
plt.plot(preds, label="Prédiction (from scratch)", color="orangered",
         linewidth=2, linestyle="--")
plt.title("Prédictions du RNN from scratch sur un sinus")
plt.xlabel("Pas de temps")
plt.ylabel("y(t)")
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig("rnn_scratch_predictions.png", dpi=150, bbox_inches="tight")
plt.show()
...
our_outputs, our_hiddens = rnn.forward(X_verify)

jax_all_h = jax_rnn_forward(rnn.W_xh, rnn.W_hh, rnn.b_h, X_verify, seq_length)
